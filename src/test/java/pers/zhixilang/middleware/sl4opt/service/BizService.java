...
public class BizService {
    ...
    @Sl4opt(success = "hello @#name@, 结果=@#_res@", bizType = "1", fail = "失败: @#_err@")
    public String variableDo() {
        Sl4optContext.putVariable("name", "zs");
        return "success";
    }

    @Sl4opt(success = "新增用户「@#person.name@」成功, ID=「@id_func@」", bizType = "1")
    public void noArgFunctionDo(Person person) {
        System.out.println("...");
    }


    @Sl4opt(success = "新增用户「@#person.name@」成功, ID=「@id_func@」，自动生成昵称「@nick_func#person.name@」", bizType = "1")
    public void argFunctionDo(Person person) {
        System.out.println("...");
    }


    @Sl4opt(success = "新增用户「@#person.name@」成功, 自动生成昵称「@nick_func#person.name,#person.age@」", bizType = "1")
    public void multiArgFunctionDo(Person person) {
        System.out.println("...");
    }

    @Sl4opt(success = "新增用户「@#person.name@」成功, ID=「@#_res.id@」 自动生成昵称「@nick_func#person.name,#person.age@」", bizType =
            "1", fail = "新增用户「@#person.name@」失败：@#_err@")
